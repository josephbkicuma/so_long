...
/*   Updated: 2024/08/16 12:02:53 by jquicuma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill(t_mlx *tab, t_point size, t_point cur)
{
	if (cur.y < 0 || cur.y >= size.y || cur.x < 0 || cur.x >= size.x
		|| (tab->map[cur.y][cur.x] != 'P' && tab->map[cur.y][cur.x] != 'C'
			&& tab->map[cur.y][cur.x] != 'E' && tab->map[cur.y][cur.x] != '0'))
		return ;
...
void	fill_reverse(t_mlx *tab, t_point size, t_point cur)
{
	if (cur.y < 0 || cur.y >= size.y || cur.x < 0 || cur.x >= size.x
		|| (tab->map[cur.y][cur.x] != 'J' && tab->map[cur.y][cur.x] != 'L'
			&& tab->map[cur.y][cur.x] != 'X' && tab->map[cur.y][cur.x] != 'Z'))
		return ;
...
int	valid_access(t_mlx *var, t_point size)
{
	...
	begin.y = pos[0];
	fill(var, size, begin);
	if (count_elements(var->map, 'E') > 0 || count_elements(var->map, 'C') > 0)
		return (0);
